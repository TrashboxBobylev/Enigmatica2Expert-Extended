    ...
    for b in 0 .. 6 {
      for c in 0 .. 6 {
        if (a % 5 == 0 || b % 5 == 0 || c % 5 == 0) {
          ...
          val block as IBlock = target.world.getBlock(pos);
          if (!isNull(block)
            && target.world.getBlockState(pos).isReplaceable(target.world, pos)) {
            target.world.setBlockState(ancientBar, pos);
          }
        }
      }
    }
...
function sonusSplit(scythe as IEntity, target as IEntityLivingBase, lvl as int) as void {
  ...
  for entity in entitiesList {
    if (isNull(entity)
      || !entity instanceof IEntityLiving
      || target.getDistanceSqToEntity(entity) > 20 + lvl * 5) {
      continue;
    }
...
function scytheEffectElemental(scythe as IEntity, target as IEntityLivingBase, player as IPlayer, augments as string[], dmg as double, colorCount as int[]) as void {
  ...
  if (augments has 'mana')         {
    if (augments has 'rattus') { scytheCreateChaos(scythe, target); }
    else { manaCreateStar(scythe, target); }
  }
